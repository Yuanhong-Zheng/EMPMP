import torch
import numpy as np
from src.models_trajectory.utils import predict,split_pose_and_trjectory,cal_final_pose
...
def mpjpe_test_regress(config, model, eval_generator,dataset="mocap"):    
    ...
    for (h36m_motion_input, h36m_motion_target) in eval_generator:
        ...
        pose_pred,trajectory_target =predict(model,pose_input,trajectory_input,config)
        motion_pred=cal_final_pose(pose_pred,trajectory_target)
