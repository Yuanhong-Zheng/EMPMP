...
from src.models_trajectory.model import siMLPe as Model
from src.models_trajectory.utils import visuaulize,seed_set,get_dct_matrix,gen_velocity,predict,getRandomPermuteOrder,getRandomRotatePoseTransform,split_pose_and_trjectory,cal_final_pose
from lr import update_lr_multistep
print('当前工作目录：',os.getcwd())
from src.baseline_h36m_30to30_trajectory.lib.datasets.dataset_mocap import DATA
from lib.utils.logger import get_logger, print_and_log_info
from lib.utils.pyt_utils import  ensure_dir
import torch
from torch.utils.tensorboard import SummaryWriter
from src.baseline_h36m_30to30_trajectory.test import mpjpe_test_regress,regress_pred
import shutil

parser = argparse.ArgumentParser(formatter_class=argparse.ArgumentDefaultsHelpFormatter)
parser.add_argument('--exp-name', type=str, default="30to30,baseline,trj方式,k=0.8,num=32", help='=exp name')
...
parser.add_argument('--num', type=int, default=32, help='=num of blocks')
parser.add_argument('--k', type=float, default=0.8, help='=num of blocks')
...
def train_step(h36m_motion_input, h36m_motion_target, model, optimizer, nb_iter, total_iter, max_lr, min_lr) :
    ...
    pose_input,trajectory_input=split_pose_and_trjectory(h36m_motion_input.clone())#b,p,t,jk  b,p,t,k
    pose_target,trajectory_target=split_pose_and_trjectory(h36m_motion_target.clone())
    
    pose_pred,trajectory_pred=predict(model,pose_input,trajectory_input,config)#b,p,t,jk  b,p,t,k
    motion_pred=cal_final_pose(pose_pred,trajectory_pred)#b,p,t,jk
    b,p,n,c = h36m_motion_target.shape
    
    #预测的姿态
    pose_pred = pose_pred.reshape(b,p,n,config.n_joint,3).reshape(-1,3)#bptj,k
    #GT
    pose_target = pose_target.to(config.device).reshape(b,p,n,config.n_joint,3).reshape(-1,3)#bptj,k
    #计算loss
    loss_pose = torch.mean(torch.norm(pose_pred - pose_target, 2, 1))
    
    #计算trajectory的loss
    trajectory_pred=trajectory_pred.reshape(-1,3)#bpt,k
    trajectory_target=trajectory_target.to(config.device).reshape(-1,3)#bpt,k
    loss_trajectory = torch.mean(torch.norm(trajectory_pred - trajectory_target, 2, 1))
    
    k=args.k
    loss=k*loss_pose+(1-k)*loss_trajectory
    
    if config.use_relative_loss:
        motion_pred = motion_pred.reshape(b,p,n,config.n_joint,3)
        dmotion_pred = gen_velocity(motion_pred)#计算速度
        
        motion_gt = h36m_motion_target.reshape(b,p,n,config.n_joint,3).to(config.device)
...
while (nb_iter + 1) < config.cos_lr_total_iters:
    ...
    for (h36m_motion_input, h36m_motion_target) in data_source:
        ...
        if nb_iter == 9:
            print("第十个iter的loss:",loss)#0.3655661344528198
        ...
        if (nb_iter + 1) % config.save_every ==  0 or nb_iter==0:
            with torch.no_grad():
                ...
                if config.dataset=="h36m":
                    ...
                    mpjpe_res_mocap=[np.mean(mpjpe_res_mocap)]+mpjpe_res_mocap
                    ...
                    mpjpe_res_mupots=[np.mean(mpjpe_res_mupots)]+mpjpe_res_mupots
        ...
        if ((nb_iter + 1) % config.vis_every ==  0 or nb_iter==0) and config.dataset!="h36m":
            model.eval()
            with torch.no_grad():  
                h36m_motion_input = eval_dataset_mupots.sample()[:,:,:config.t_his]
                ...
                pose_input,trajectory_input=split_pose_and_trjectory(h36m_motion_input.clone())#b,p,t,jk  b,p,t,k
                pose_target,trajectory_target=predict(model,pose_input,trajectory_input,config)
                motion_pred=cal_final_pose(pose_target,trajectory_target)
