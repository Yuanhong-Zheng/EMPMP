...
import json
from src.models_inter_dual_out.utils import visuaulize,seed_set,get_dct_matrix,gen_velocity,predict,update_metric,getRandomPermuteOrder,getRandomRotatePoseTransform
from lr import update_lr_multistep
from src.baseline_3dpw_dual_out.config import config
from src.models_inter_dual_out.model import siMLPe as Model
from src.baseline_3dpw_dual_out.lib.dataset.dataset_3dpw import get_3dpw_dataloader
from src.baseline_3dpw_dual_out.lib.utils.logger import get_logger, print_and_log_info
from src.baseline_3dpw_dual_out.lib.utils.pyt_utils import  ensure_dir
import torch
from torch.utils.tensorboard import SummaryWriter
from src.baseline_3dpw_dual_out.test import vim_test,random_pred
import shutil
...
parser.add_argument('--exp-name', type=str, default="3dpw数据集,inter_dual_out,k=0.8,first_norm,interval=16,random_rotate,permute_p", help='=exp name')
...
parser.add_argument('--vis_every', type=int, default=200000000)
...
def train_step(h36m_motion_input, h36m_motion_target,padding_mask, model, optimizer, nb_iter, total_iter, max_lr, min_lr) :
    ...
    motion_pred,motion_pred_global=predict(model,h36m_motion_input,config)#b,p,n,c
    b,p,n,c = h36m_motion_target.shape
    #预测的姿态
    motion_pred = motion_pred.reshape(b,p,n,config.n_joint,3).reshape(-1,3)
    motion_pred_global=motion_pred_global.reshape(b,p,n,config.n_joint,3).reshape(-1,3)
    #GT
    ...
    #计算loss
    loss1 = torch.mean(torch.norm(motion_pred - h36m_motion_target, dim=1)[expanded_mask])
    loss2=torch.mean(torch.norm(motion_pred_global - h36m_motion_target, dim=1)[expanded_mask])
    k=0.8
    loss=k*loss1+(1-k)*loss2
