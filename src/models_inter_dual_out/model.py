...
class siMLPe(nn.Module):
    def __init__(self, config):
        ...
        if self.temporal_fc_out:
            ...
            self.motion_fc_out_global = nn.Linear(self.config.motion.dim, self.config.motion.dim)
    ...
    def forward(self, motion_input):
        ...
        motion_feats,motion_feats_global = self.motion_mlp(motion_feats)
        ...
        if self.temporal_fc_out:
            ...
            motion_feats_global=motion_feats_global.reshape(b,d,p,t).permute(0,2,3,1)#b,p,t,d
            motion_feats_global=self.motion_fc_out_global(motion_feats_global)

        return motion_feats,motion_feats_global
