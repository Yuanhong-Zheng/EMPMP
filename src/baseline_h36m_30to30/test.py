...
def regress_pred(model,motion_input,config):
    ...
    # joint_equal = np.array([13, 19, 22, 13, 27, 30]).astype(np.int64)
        ...
        motion_pred = torch.cat(outputs, axis=2)[:,:,:30]
...
def mpjpe_test_regress(config, model, eval_generator,dataset="mocap"):    
    ...
    for (h36m_motion_input, h36m_motion_target) in eval_generator:
        ...
        #p,t,j,k
        if dataset=="mocap":
            loss1=torch.sqrt(((motion_pred[:,:10]/1.8 - h36m_motion_target[:,:10]/1.8) ** 2).sum(dim=-1)).mean(dim=-1).mean(dim=-1).numpy().tolist()
            loss2=torch.sqrt(((motion_pred[:,:20]/1.8 - h36m_motion_target[:,:20]/1.8) ** 2).sum(dim=-1)).mean(dim=-1).mean(dim=-1).numpy().tolist()
            loss3=torch.sqrt(((motion_pred[:,:30]/1.8 - h36m_motion_target[:,:30]/1.8) ** 2).sum(dim=-1)).mean(dim=-1).mean(dim=-1).numpy().tolist()

        else:
            loss1=torch.sqrt(((motion_pred[:,:10] - h36m_motion_target[:,:10]) ** 2).sum(dim=-1)).mean(dim=-1).mean(dim=-1).numpy().tolist()
            loss2=torch.sqrt(((motion_pred[:,:20] - h36m_motion_target[:,:20]) ** 2).sum(dim=-1)).mean(dim=-1).mean(dim=-1).numpy().tolist()
            loss3=torch.sqrt(((motion_pred[:,:30] - h36m_motion_target[:,:30]) ** 2).sum(dim=-1)).mean(dim=-1).mean(dim=-1).numpy().tolist()
