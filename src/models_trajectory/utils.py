...
def getRandomPermuteOrder(joints_input, joints_target):
    ...
    return joints_input,joints_target
def split_pose_and_trjectory(motion):
    b,p,t,jk=motion.shape
    k=3
    j=jk//k
    motion=motion.reshape(b,p,t,j,k)
    trajectory=motion[:,:,:,:1,:]#根节点的运动轨迹
    pose=motion-trajectory#pose
    
    trajectory=trajectory.flatten(-2)#b,p,t,k
    pose=pose.flatten(-2)#b,p,t,jk
    return pose,trajectory

def cal_final_pose(pose,trajectory):#b,p,t,jk b,p,t,k
    b,p,t,jk=pose.shape
    pose=pose.reshape(b,p,t,-1,3)#b,p,t,j,k
    trajectory=trajectory.unsqueeze(3)#b,p,t,1,k
    final_pose=pose+trajectory#b,p,t,j,k
    final_pose=final_pose.reshape(b,p,t,jk)#b,p,t,jk
    return final_pose

def predict(model,h36m_motion_input,trajectory_input,config):
    ...
    motion_pred,trajectory_pred = model(h36m_motion_input_.to(config.device),trajectory_input.to(config.device))#b,p,t,jk  b,p,t,k
    ...
    if config.normalization:
        ...
        trajectory_pred=trajectory_pred[:,:, :config.t_pred]
        
        motion_offset=h36m_motion_input[:,:,-1:].to(config.device)
        motion_pred=motion_pred+motion_offset
        
        trajectory_offset=trajectory_input[:,:,-1:].to(config.device)
        trajectory_pred=trajectory_pred+trajectory_offset
        # offset = h36m_motion_input[:, :,-1:].to(config.device)#b,p,1,c
        # motion_pred = motion_pred[:,:, :config.t_pred] + offset#b,p,n,c
    return motion_pred,trajectory_pred
